...
@section('main')
        ...
        <div class="row justify-content-center d-flex mt-5">
            <div class="col-md-12">
                ...
                <div class="row mt-4">
                    <div class="col-md-4">
                        @if ($book->image != null)
                            <img src="{{ asset('uploads/book/' . $book->image) }}" alt="" class="card-img-top">
                        @else
                            ...
                            @if ($relatedBooks->isNotEmpty())
                                @foreach ($relatedBooks as $rbook)
                                    <div class="col-md-4 col-lg-4 mb-4">
                                        <a href="{{ route('book-detail', $rbook->id) }}" class="text-decoration-none">
                                            <div class="card border-0 shadow-lg">
                                                @if ($rbook->image != null)
                                                    <img src="{{ asset('uploads/book/' . $rbook->image) }}" alt=""
                                                        class="card-img-top">
                                                @else
                                                ...
                                                <div class="card-body">
                                                    <h3 class="h4 heading">{{ $rbook->name }}</h3>
                                                    <p>by {{ $rbook->author }}</p>
                                                    <div class="star-rating d-inline-flex ml-2" title="">
                                            ...
                                            <div class="card-body">
                                                <div class="d-flex justify-content-between">
                                                    <h5 class="mb-3">{{ $review->user->name }}</h4>
                                                        <span
                                                            class="text-muted">{{ \Carbon\Carbon::parse($review->created_at)->diffForHumans() }}</span>
                                                </div>
                                                @php
                                                    $ratingPercent = ($review->rating / 5) * 100;
                                                @endphp
    ...
    <div class="modal fade " id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog modal-dialog-centered">
            ...
            @section('script')
                <script>
                    $('#book-review-form').on('submit', function(e) {
                        e.preventDefault();
                        $.ajax({
                            url: '{{ route('save-review') }}', // Add the correct URL here
                            type: 'POST',
                            headers: {
